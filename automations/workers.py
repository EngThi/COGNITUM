...
from automations.state import get_unprocessed_events, mark_event_status, save_event
from automations.log import get_logger
from automations.router import process_router_queue, generate_with_backoff, get_genai_client, clean_json_text
...
"""
            ...
            if flashcards:
                ...
                async with aiosqlite.connect(settings.database_path) as db:
                    ...
                    await db.commit()
                    logger.info(f"[Flashcard Worker] Inserted {len(flashcards)} flashcards into flashcards_state DB.")
    ...
    for event in events:
        ...
        try:
            ...
            if due_cards:
                ...
                # Mock spacing update (basic scheduler loop) for the due cards
                # Increment interval by 1 day per review
                async with aiosqlite.connect(settings.database_path) as db:
...
def is_restricted_hours() -> bool:
    ...
    try:
        ...
        # Restricted: 22:30 (1350 mins) to 06:10 (370 mins)
        if now_mins >= 1350 or now_mins <= 370:
            return True
        return False
    except Exception as e:
        logger.error(f"Error checking restricted hours: {e}")
        return False
...
async def get_last_kimi_use() -> datetime | None:
    try:
        ...
        logger.error(f"Error reading last_kimi_use.txt: {e}")
    return None
...
async def manage_kimiproxy_service():
    ...
    try:
        ...
        if should_run and not is_active:
            ...
            logger.info("[KimiProxy Manager] Stopping KimiProxy service due to restricted hours inactivity...")
