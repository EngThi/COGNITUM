...
async def record_kimi_use():
    ...
    try:
        ...
        logger.error(f"Failed to record Kimi use: {e}")
...
async def ensure_kimiproxy_running():
    ...
    from automations.log import get_logger
    logger = get_logger("state")
    kimi_url = os.environ.get("KIMI_PROXY_URL", "http://localhost:3000")
    ...
    try:
        ...
        if res.stdout.strip() != "active":
            logger.info("Waking up KimiProxy service...")
            subprocess.run(["sudo", "systemctl", "start", "kimiproxy"], capture_output=True)
            await asyncio.sleep(6)
    except Exception as e:
        logger.error(f"Error ensuring KimiProxy running: {e}")
